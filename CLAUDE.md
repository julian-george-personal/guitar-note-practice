...
**Exercise pattern**: Each exercise (`NoteExercise`, `StringExercise`, `CustomExercise`) receives `audio: AudioData` as a prop. It picks a target, uses `useNoteMatch` to compare detected vs. target, and calls `advance` when the user holds the correct note for 200 ms. `useNoteMatch` triggers a 400 ms "correct" visual before advancing.

**Custom exercise (AI)**: `CustomExercise` sends a user prompt to a Lambda (`VITE_FRETBOARDER_LAMBDA_URL`) which calls AWS Bedrock (Claude Haiku) and returns an `ExerciseConfig` JSON. `exercise-config.ts` defines the `ExerciseConfig` schema, `generateExerciseConfig`, and pool/step-pattern helpers. The Lambda handler lives in `job/src/index.ts` and is deployed separately from the web app.
...
**Storage**: All `localStorage` access goes through `src/storage.ts`. Never call `localStorage` directly — use `storage.scale`, `storage.tuning`, `storage.fretRange`, `storage.enabledStrings`, `storage.exerciseConfig`, `storage.exercisePrompt`.
